  ...
  try {
    const result = await client.request({
      ...
      params: {
        ...
        arguments: {
          projectName: "Antigravi-platadv",
          projectPath: "c:/Users/victo/OneDrive/Documentos/Antigravi-platadv",
          testIds: [],
          additionalInstruction: "General backend coverage. Ensure all core modules (Auth, Clients, Financial, Processes) are tested. The backend is running on port 3000.",
          serverMode: "development"
        }
      }
    }, CallToolResultSchema, { timeout: 1200000 }); // 20 minutes timeout
    
    console.log(JSON.stringify(result, null, 2));
  } catch (err) {
    console.error("Execution error or timeout:", err);
  } finally {
