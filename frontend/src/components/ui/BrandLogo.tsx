...
export function BrandLogo({ className = '', variant = 'dark', size = 'md', showText = true }: BrandLogoProps) {
    ...
    return (
        <div className={`flex items-center ${currentSize.gap} font-display select-none group ${className}`}>
            <div className={`relative ${currentSize.box} flex items-center justify-center shrink-0 overflow-hidden bg-gradient-to-br from-primary via-primary-dark to-black shadow-xl shadow-primary/20 border border-white/10 group-hover:scale-105 transition-transform duration-500`}>
                {/* Decorative Elements */}
